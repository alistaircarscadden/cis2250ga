package statsIndexGenerator;

use Exporter;
@ISA    = ('Exporter');
@EXPORT = ('generateStatsIndex');

use strict;
use warnings;
use version; our $VERSION = qv('5.16.0');

sub generateStatsIndex {
    my ($records_ref, $col0_ref, $col1_ref, $col2_ref, $col3_ref, $col4_ref, $col5_ref, $col6_ref) = @_;
    
    my @records = @{$records_ref};
    my @col0    = @{$col0_ref};      # year
    my @col1    = @{$col1_ref};      # loc
    my @col2    = @{$col2_ref};      # violation type
    my @col3    = @{$col3_ref};      # stat type
    my @col4    = @{$col4_ref};      # v
    my @col5    = @{$col5_ref};      # coordinate
    my @col6    = @{$col6_ref};      # value
    
    my $max_stat_number = 14;
    my @unique_stats;
    
    # Initialize $max_stat_number strings into @unique_violations
    for ( my $i = 0 ; $i < $max_stat_number ; $i++ ) {
        $unique_stats[$i] = 'STATDOESNOTEXIST';
    }
    
    for ( my $i = 0 ; $i < @records ; $i++ ) {
        my $unique = 1;
        
        # Check if the violation on current line is unique
        foreach my $unique_stat (@unique_stats) {
            if ( $col3[$i] eq $unique_stat ) {
                $unique = 0;
            }
        }
        
        if ( $unique == 1 ) {
            my @coordinates = split( /\./, $col5[$i] );
            $unique_stats[ $coordinates[2] - 1 ] = $col3[$i];
        }
    }
    
    open my $output_fh, '>', 'stats_index.csv' or die $!;
    for my $str (@unique_stats) {
        print $output_fh '"' . $str . "\"\n";
    }
    close $output_fh or die $!;
}
